...
if st.sidebar.checkbox('Show a random Test image from MNIST'):
    ...
    image = x_test[num]
    st.sidebar.image(image, caption=class_names[y_test[num]], width=192)
...
if st.checkbox('Show random image from the test set'):
    num = np.random.randint(0, x_test.shape[0])
    image = x_test[num]
    st.image(image, caption=class_names[y_test[num]], width=96)
...
if st.checkbox('Show 10 different image from the test set'):
    num_10 = np.unique(y_test, return_index=True)[1]
    images = x_test[num_10]
    for i in range(len(images)):
        # define subplot
        plt.subplot(2,5,1 + i) #, sharey=False)
